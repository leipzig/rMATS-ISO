...
int add_exon(trans_t *t, exon_t *e)
{
    if (t->exon_n == t->exon_m) t = exon_realloc(t);
    t->exon[t->exon_n].tid = e->tid;
    t->exon[t->exon_n].start = e->start;
    t->exon[t->exon_n].end = e->end;
    t->exon[t->exon_n].is_rev = e->is_rev;
    strcpy(t->exon[t->exon_n].tname, e->tname);
    strcpy(t->exon[t->exon_n].trans_id, e->trans_id);
    strcpy(t->exon[t->exon_n].gname, e->gname);
    strcpy(t->exon[t->exon_n].gid, e->gid);
    t->exon_n++;
    return 0;
}
...
void add_read_trans(read_trans_t *r, trans_t t)
{
    ...
    for (i = 0; i < t.exon_n; ++i)
        add_exon(r->t+r->trans_n, t.exon+i);
...
void add_trans(gene_t *g, trans_t *t, int novel_gene_flag)
{
    ...
    strcpy(g->trans[g->trans_n].gname, t->gname);
    strcpy(g->trans[g->trans_n].gid, t->gid);

    g->trans[g->trans_n].exon_n = 0;
    for (i = 0; i < t->exon_n; ++i) {
        add_exon(g->trans+g->trans_n, t->exon+i);
    }
...
int has_common_site(gene_t *g1, gene_t *g2) {
    if (g1->tid != g2->tid) return 0;
    int i, j, k, m;
    int don, acc;
    for (i = 0; i < g1->trans_n; ++i) {
        if (g1->trans[i].exon_n < 2) continue;
        for (j = 0; j < g1->trans[i].exon_n-1; ++j) {
            don = g1->trans[i].exon[j].end;
            acc = g1->trans[i].exon[j+1].start;
            if (g2->start > acc || g2->end < don) continue;
            for (k = 0; k < g2->trans_n; ++k) {
                if (g2->trans[k].exon_n < 2 || g2->trans[k].start > acc || g2->trans[k].end < don) continue;
                for (m = 0; m < g2->trans[k].exon_n-1; ++m) {
                    if (don == g2->trans[k].exon[m].end || acc == g2->trans[k].exon[m+1].start)
                        return 1;
                }
            }
        }
...
void merge_gene(gene_t *dest, gene_t *src) {
    if (dest == src) return;
    int i;
    if (src->start < dest->start) dest->start = src->start;
    if (src->end > dest->end) dest->end = src->end;
    for (i = 0; i < src->trans_n; ++i) {
        add_trans(dest, src->trans+i, 0);
    }
    src->trans_n = 0;
}

void merge_overlapping_gene(gene_group_t *gg) {
    int i, j;
    int merged_gene_n = 0;
    for (i = 1; i < gg->gene_n; ++i) {
        for (j = i-1; j >= 0; --j) {
            if (gg->g[j].tid < gg->g[i].tid || gg->g[j].end <= gg->g[i].start) break;
            else if (has_common_site(gg->g+j, gg->g+i)) {
                merge_gene(gg->g+j, gg->g+i);
                merged_gene_n++;
                break;
            }
        } 
    }
    gg->gene_n -= merged_gene_n;
}

void get_overlap_reg(gene_group_t *gg) {
    int i, j;
    for (i = 0; i < gg->gene_n-1; ++i) {
        for (j = i+1; j < gg->gene_n; ++j) {
            if (gg->g[i].tid < gg->g[j].tid || gg->g[i].end <= gg->g[i].start) break;
            int ii = gg->g[i].ovlp_n, jj = gg->g[j].ovlp_n;
            _uni_realloc(gg->g[i].ovlp_reg, gg->g[i].ovlp_n, gg->g[i].ovlp_m, reg_t);
            _uni_realloc(gg->g[j].ovlp_reg, gg->g[j].ovlp_n, gg->g[j].ovlp_m, reg_t);

            gg->g[i].ovlp_reg[ii].start = gg->g[j].ovlp_reg[jj].start = MAX_OF_TWO(gg->g[i].start, gg->g[j].start);
            gg->g[i].ovlp_reg[ii].end = gg->g[j].ovlp_reg[jj].end = MIN_OF_TWO(gg->g[i].end, gg->g[j].end);
            gg->g[i].ovlp_n++;
            gg->g[j].ovlp_n++;
        }
    }
...
/*int read_gene_group(char *fn, chr_name_t *cname, gene_group_t *gg)
{
    ...
    while (fgets(line, 1024, gtf) != NULL) {
        ...
        if (strcmp(type, "gene") == 0) { // new gene starts old gene ends
            //if (tid == last_tid &&  start < last_end) { // merge two genes only they share at least one splice site
            //    if (st==art < last_start) {
            //        la==st_start = start;
            //        cu==r_g->start = start;
            //    }
            //    if (en==d > last_end) {
            ...
            strcpy(cur_t->gname, gname); strcpy(cur_t->gid, gid);
            cur_t->exon_n = 0;
            ...
            strcpy(cur_t->tname, trans_name); strcpy(cur_t->trans_id, trans_id);
            strcpy(cur_e->gname, gname); strcpy(cur_e->gid, gid);
        }
    }
    // reverse '-' transcript
...
int read_gene_group(char *fn, chr_name_t *cname, gene_group_t *gg)
{
    ...
    gene_t *cur_g=0; trans_t *cur_t=0; exon_t *cur_e=0;
    gg->gene_n = 0;
    char last_gname[100]="\0", last_tname[100]="\0";
    //int last_tid=-1, last_start=-1, last_end=-1;
    ...
    while (fgets(line, 1024, gtf) != NULL) {
        ...
        //if (strcmp(type, "gene") == 0) { // new gene starts old gene ends
        if (strcmp(gname, last_gname) != 0) { // new gene
            //if (tid == last_tid &&  start < last_end) { // merge two genes only they share at least one splice site
            //    if (st==art < last_start) {
            //        la==st_start = start;
            //        cu==r_g->start = start;
        ...
        if (start < cur_t->start) cur_t->start = start;
        if (start < cur_g->start) cur_g->start = start;
        if (end > cur_t->end) cur_t->end = end;
        if (end > cur_g->end) cur_g->end = end;

        strcpy(cur_e->tname, trans_name); strcpy(cur_e->trans_id, trans_id);
        strcpy(cur_e->gname, gname); strcpy(cur_e->gid, gid);
    }
    // reverse '-' transcript
    reverse_exon_order(gg);
